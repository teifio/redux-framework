		...
		public static function instance(): ?Redux_Framework_Plugin {
			...
			if ( function_exists( 'get_plugin_data' ) && file_exists( $path ) ) {
				...
				if ( true === $res && ! in_array( 'redux-framework/redux-framework.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
					echo '<div class="error"><p>' . esc_html__( 'Redux Framework version 4 is activated but not loaded. Redux Framework version 3 is still installed and activated.  Please deactivate Redux Framework version 3.', 'redux-framework' ) . '</p></div>';
					return null;
				}
			}
